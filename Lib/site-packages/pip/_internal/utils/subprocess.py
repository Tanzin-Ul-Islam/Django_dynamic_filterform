...
def make_subprocess_output_error(
    ...
    # Convert `commands` and `cwd` to text (unicode in Python 2) so we can use
    # them as arguments in the unicode format string below. This avoids
    # "UnicodeDecodeError: 'ascii' codec can't decode byte ..." in Python 2
    # if either contains a non-ascii character.
    command_display = str_to_display(command, desc='commands bytes')
    ...
    try:
        ...
        if log_failed_cmd:
            subprocess_logger.critical(
                "Error %s while executing commands %s", exc, command_desc,
            )
        raise
    all_output = []
    while True:
    ...
    if proc_had_error:
        if on_returncode == 'raise':
            ...
            exc_msg = (
                ...
                'Check the logs for full commands output.'
